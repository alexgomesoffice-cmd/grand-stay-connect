...
const MyBookings = () => {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem("stayvista-bookings");
    if (saved) {
      try {
        const parsed: MyBooking[] = JSON.parse(saved);
        setBookings(parsed);
      } catch {
        setBookings([]);
      }
        ...
        {bookings.length > 0 ? (
          <div className="space-y-4">
            {bookings.map((booking, i) => {
              ...
              return (
                <Card key={booking.id} className="hover-lift cursor-pointer animate-fade-in-up" style={{ animationDelay: `${i * 80}ms` }}>
